...
polygon::polygon(int num_vert)
{
	...
	set_point_array(vert);                  // есть конструктор, а есть инициализация, возможно, стоит разделить (+init)
}
...
bool polygon::is_regular(bool convexity) const//не нужно ли передавать переменную по ссылке? кажется, она не меняется, нужен конст
{
